        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
          if (tabs[0]) {
            chrome.tabs.sendMessage(tabs[0].id, { action: "toggleSidebar" }, (response) => {
              if (chrome.runtime.lastError) {
                console.error("Popup: Error sending toggleSidebar message:", chrome.runtime.lastError.message);
                displayMessage("Failed to toggle Sidebar.");
              } else {
                displayMessage(response.visible ? "Sidebar opened." : "Sidebar closed.");
              }
            });
          } else {
