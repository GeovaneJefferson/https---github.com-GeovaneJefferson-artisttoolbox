...
const envMapFileNameDisplay = document.getElementById('envMapFileNameDisplay');
const envMapLoadingSpinner = document.getElementById('envMapLoadingSpinner');
const envMapPreviewCanvas = document.getElementById('envMapPreviewCanvas');
const envMapPreviewCtx = envMapPreviewCanvas ? envMapPreviewCanvas.getContext('2d') : null;
const applyEnvMapButton = document.getElementById('applyEnvMapButton');
const envMapDropZone = document.getElementById('envMapDropZone');
let envMapImage = null;

// Setup drop zone for env map
if (envMapDropZone && envMapInput) {
    setupDropZone(envMapDropZone, envMapInput);
}

// Handle file input and preview
if (envMapInput) {
    envMapInput.addEventListener('change', (e) => {
        const file = e.target.files[0];
        if (!file) {
            envMapFileNameDisplay.textContent = 'No image selected.';
            envMapImage = null;
            if (envMapPreviewCtx) envMapPreviewCtx.clearRect(0, 0, envMapPreviewCanvas.width, envMapPreviewCanvas.height);
            return;
        }
        envMapFileNameDisplay.textContent = file.name;
        envMapLoadingSpinner.style.display = 'block';
        const reader = new FileReader();
        reader.onload = function(ev) {
            const img = new Image();
            img.onload = function() {
                envMapLoadingSpinner.style.display = 'none';
                envMapImage = img;
                // Draw preview
                if (envMapPreviewCtx) {
                    envMapPreviewCtx.clearRect(0, 0, envMapPreviewCanvas.width, envMapPreviewCanvas.height);
                    // Fit image to canvas
                    const aspect = img.width / img.height;
                    let w = envMapPreviewCanvas.width, h = envMapPreviewCanvas.height;
                    if (w / h > aspect) {
                        w = h * aspect;
                    } else {
                        h = w / aspect;
                    }
                    envMapPreviewCtx.drawImage(img, (envMapPreviewCanvas.width-w)/2, (envMapPreviewCanvas.height-h)/2, w, h);
                }
            };
...
// Apply as environment map
if (applyEnvMapButton) {
    applyEnvMapButton.addEventListener('click', () => {
        if (!envMapImage || !scene) {
            showMessageBox('Please upload an image first.', 'error');
            return;
        }
        // Create a texture from the image
        const texture = new THREE.Texture(envMapImage);
        texture.needsUpdate = true;
        texture.mapping = THREE.EquirectangularReflectionMapping;
        // Set as scene background and environment
        scene.background = texture;
        scene.environment = texture;
        showMessageBox('Environment map applied!', 'success');
    });
}
...
const emissionMapCanvas = document.getElementById('emissionMapCanvas');
...
const exportButtonDropdown = document.getElementById('exportButtonDropdown');
...
const exportEmissionMapDropdownCheckbox = document.getElementById('exportEmissionMapDropdown');
...
const emissionValueDisplay = document.getElementById('emissionThresholdValue');
...
const emissionMapPreviewCanvas = document.getElementById('emissionMapCanvas');
const emissionMapPreviewCtx = emissionMapPreviewCanvas.getContext('2d');
...
const fullResEmissionCanvas = document.createElement('canvas');
const fullResEmissionCtx = fullResEmissionCanvas.getContext('2d');
const pristineSourceCanvas = document.createElement('canvas');
    ...
    reader.onload = function(e) {
        ...
        img.onload = function() {
            ...
            if (fileInput.id === 'textureInput') {
                ...
                meshLocalImage = img; // Store image specifically for Image to Mesh tab
            }
...
function updateThreeJsTextures() {
    ...
    textureWeaverMaterial.emissiveMap = createBaseTexture(fullResEmissionCanvas);
    textureWeaverMaterial.emissive = new THREE.Color(0xffffff); // Make sure emissive is enabled
    textureWeaverMaterial.color = new THREE.Color(0xffffff);
    textureWeaverMaterial.emissiveIntensity = 1.0; // You can increase this for more glow
    ...
    if (texturedGroundMaterial && sharedImage) {
        ...
        texturedGroundMaterial.emissiveMap = setupGroundTexture(textureWeaverMaterial.emissiveMap);
        ...
        texturedGroundMaterial.emissive.copy(textureWeaverMaterial.emissive);
        texturedGroundMaterial.emissiveIntensity = textureWeaverMaterial.emissiveIntensity;
        texturedGroundMaterial.color.set(0xffffff); // Ensure ground color is also white
        ...
        texturedGroundMaterial.emissiveMap = null;
    }
...
function set3DPreviewMode(mapType) {
    ...
    if (mapType === 'Color Map' || (container && container.classList.contains('isolated-view'))) {
    ...
    const sourceCanvasMap = {
        'Color Map': fullResOriginalCanvas, // Added original texture to map for consistent handling
        ...
        'Emission Map': fullResEmissionCanvas
    };
    ...
    if (sourceCanvas) {
        ...
        // Only Albedo (Color Map) should have sRGB encoding.
        planePreviewTexture.encoding = (mapType === 'Color Map') ? THREE.sRGBEncoding : THREE.LinearEncoding;
        ...
        if (mapType === 'Emission Map') {
            textureWeaverMaterial.map = null;
            textureWeaverMaterial.emissiveMap = planePreviewTexture;
            textureWeaverMaterial.emissive = new THREE.Color(0xffffff);
            textureWeaverMaterial.emissiveIntensity = 1.0;
            textureWeaverMaterial.color = new THREE.Color(0x000000);
        } else {
            textureWeaverMaterial.map = planePreviewTexture;
            textureWeaverMaterial.emissiveMap = null;
            textureWeaverMaterial.emissive = new THREE.Color(0x000000);
            textureWeaverMaterial.emissiveIntensity = 1.0;
            textureWeaverMaterial.color = new THREE.Color(0xffffff);
        }
...
function generateMaps() {
    if (!sharedImage) {
        ...
        clearCanvas(metallicCtx);
        clearCanvas(displacementCtx);
        clearCanvas(emissionMapCanvas.getContext('2d'));
        return;
    }
    ...
    generateEmissionMap(fullResOriginalCanvas, fullResEmissionCanvas, parseInt(emissionSlider.value));
...
function updatePreviewCanvases() {
    ...
    copyToPreview(fullResEmissionCanvas, emissionMapPreviewCtx);
}
 ...
 * @param {number} sharpness - Controls the threshold for what is considered metallic. 1.0 is fully metallic, 0.0 is non-metallic.
...
function generateMetallicMap(sourceCanvas, targetCanvas, sharpness, invert) {
    ...
    // The threshold determines how bright a pixel needs to be to be considered metallic.

    // A high sharpness (e.g., 1.0) means a low threshold, making more pixels metallic (white).
    // A low sharpness (e.g., 0.0) means a high threshold, making fewer pixels metallic.
    const threshold = 255 * (1.0 - sharpness);

    for (let i = 0; i < data.length; i += 4) {
        const gray = toGrayscale(data[i], data[i + 1], data[i + 2]);
        
        let metallicValue = (gray > threshold) ? 255 : 0;
        
        // Invert the value if the checkbox is checked
        if (invert) {
            metallicValue = 255 - metallicValue;
        }

        newData[i] = metallicValue;
        newData[i + 1] = metallicValue;
        newData[i + 2] = metallicValue;
        newData[i + 3] = 255;
    }
    targetCtx.putImageData(newImageData, 0, 0);
}
...
/**
 * Generates an Emission Map based on the brightest parts of the image.
 * Bright areas become white (emissive), dark areas become black (non-emissive).
 * @param {HTMLCanvasElement} sourceCanvas - The canvas containing the original texture.
 * @param {HTMLCanvasElement} targetCanvas - The canvas to draw the emission map on.
 * @param {number} threshold - Value (0-255): pixels brighter than this are emissive.
 */
function generateEmissionMap(sourceCanvas, targetCanvas, threshold = 220) {
    if (!sourceCanvas || !targetCanvas || !targetCanvas.getContext('2d') || !sharedImage) {
        clearCanvas(targetCanvas.getContext('2d'));
        return;
    }
    ...
    for (let i = 0; i < data.length; i += 4) {
        const gray = toGrayscale(data[i], data[i + 1], data[i + 2]);
        const isEmissive = gray >= threshold ? 255 : 0;
        newData[i] = isEmissive;
        newData[i + 1] = isEmissive;
        newData[i + 2] = isEmissive;
        newData[i + 3] = 255;
    }
    targetCtx.putImageData(newImageData, 0, 0);
}
...
async function exportAllMaps() {
    ...
    if (exportOriginalTextureDropdownCheckbox.checked) mapsToExport.push({ canvas: fullResOriginalCanvas, name: 'color' });
    ...
    if (exportEmissionMapDropdownCheckbox.checked) mapsToExport.push({ canvas: fullResEmissionCanvas, name: 'emission' });
...
function applyOriginalStyle() {
    ...
    // Hide the pixelate settings panel and remove its active border
    pixelateStyleSettings.classList.add('hidden');
    stylePixelateBtn.classList.remove('border-blue-500', 'border-2');
}
...
function showTab(tabId) {
    ...
    // Corrected syntax error: removed extra closing brace before 'if'
    if (tabId === 'animation-combiner-tab') {
...
emissionSlider.addEventListener('input', (e) => {
    ...
    emissionValueDisplay.textContent = value;
    if (sharedImage) {
        // Regenerate the emission map with the new threshold
        generateEmissionMap(fullResOriginalCanvas, fullResEmissionCanvas, value);
        // Update the small preview
        copyToPreview(fullResEmissionCanvas, emissionMapPreviewCtx);
        // Update the 3D textures/material
        updateThreeJsTextures();
    }
});
